/* live2d-preview - a layered-PNG puppet engine (no Cubism runtime, no build).
 *
 * Draws a stack of canvas-aligned PNGs, each transformed about its own pivot by
 * a named bone. Bones are driven by mouse-follow (eye / head / body), idle
 * breath, hair + cloth springs, blink, and a held rest pose.
 *
 * Models register themselves with DCPuppet.register({...}) - see
 * models/*.model.js for the config shape. Any folder of PNGs + pivots.json can
 * also be dropped in at runtime through the folder picker.
 */
(function () {
  'use strict';

  var REG = [];
  var DCPuppet = window.DCPuppet = {
    registry: REG,
    register: function (m) { REG.push(m); }
  };
  ...
  /* Rotate + scale about (px,py), then translate by (tx,ty). */
  function local(px, py, rot, sx, sy, tx, ty) {
    ...
    var m = [a, b, cc, d,
      px + (tx || 0) - (a * px + cc * py),
      py + (ty || 0) - (b * px + d * py)];
    // A NaN pivot would silently blank the layer, so fail loud-ish instead.
    for (var i = 0; i < 6; i++) if (!isFinite(m[i])) return ID.slice();
    return m;
  }

  function blend(m, k) { // ease a transform back toward identity
    return [1 + (m[0] - 1) * k, m[1] * k, m[2] * k, 1 + (m[3] - 1) * k,
      m[4] * k, m[5] * k];
  }
  ...
  // Draw order within the figure, back to front. Role 'bg' is special-cased.
  var Z = {
    bg: 0,
    hairBack: 10, hairTip: 12, hairSide: 14,
  ...
  // Fallback pivots in normalized top-left space when a model supplies none.
  var PIV = {
  ...
  var GUESS = [
    ...
    [/(^|_)bg|background|plate|elevator/, 'bg'],
    ...
    [/(slit|flap|panel)/, 'clothPanel'],
    [/(dress|skirt|cloth|hem|robe|coat|qipao)/, 'cloth'],
    [/thigh/, 'legRaiseThigh'],
    ...
    [/(body|torso|base|cutout)/, 'base']
  ];

  // Reference / debug output that must never be treated as a puppet layer.
  var SKIP = /(composite|compare|diff|_dbg|^mask_|_probe|_raw$|raw$|bbox|report|sheet|^still)/;
  ...
  Puppet.prototype.load = function (done) {
    var self = this;
    var defs = this.model.layers.map(function (d, i) {
      var c = {};
    ...
    defs.forEach(function (d) {
      ...
      L.img.onerror = function () { self.missing.push(L.name); finish(); };
    ...
    function finish() {
      ...
      self.ready = any;
      self.hasBlinkLayer = self.layers.some(function (L) {
        return L.ok && L.role === 'blink';
      });
      done();
    }
  };
  ...
  Puppet.prototype.bones = function (s) {
    ...
    var pRoot = pv(null, 'root');
    B.root = local(pRoot[0], pRoot[1],
      s.lookX * 0.016 * o.follow, 1, 1,
    ...
    var pCh = pv(null, 'chest');
    B.chest = mul(B.root, local(pCh[0], pCh[1], 0,
    ...
    B.head = mul(B.torso, local(pHead[0], pHead[1],
      (s.lookX * 0.15 * o.follow) + s.idle * 0.35, 1, 1,
    ...
    var pCp = pv(null, 'clothPanel');
    B.clothPanel = mul(B.torso, local(pCp[0], pCp[1], s.hem * 0.055 * o.sway, 1, 1, 0, 0));
  ...
  Puppet.prototype.drawLayer = function (L, mat, alpha) {
    ...
    // Cheap non-rigid sway: shift horizontal strips further the more distant
    // they are from the pivot, so long hair / a hem curves instead of pivoting
    // as one rigid board.
  ...
  Puppet.prototype.draw = function (s) {
    ...
    // Two passes: background plates are usually fully opaque and full-canvas,
    // so they are drawn first no matter what z the config gave them.
    for (var pass = 0; pass < 2; pass++) {
      for (var i = 0; i < this.layers.length; i++) {
        var L = this.layers[i];
        if (!L.ok || !L.on) continue;
        var isBg = L.role === 'bg';
        if ((pass === 0) !== isBg) continue;
        L.bendDrive = drives[L.role] || 0;
        if (L.role === 'blink') {
          if (s.blink > 0.001) {
            this.drawLayer(L, B[L.def.follow || 'base'] || B.base, s.blink);
          }
          continue;
        }
        this.drawLayer(L, B[L.role] || B.torso, L.def.alpha);
      }
    }

    // Procedural eyelids, only for rigs with no real closed-eye plate.
    var lids = this.model.lids;
    if (lids && !this.hasBlinkLayer && s.blink > 0.02) {
      var m = B.head;
      ctx.setTransform(m[0], m[1], m[2], m[3], m[4], m[5]);
      ctx.globalAlpha = Math.min(1, s.blink * 1.15);
      for (var e = 0; e < lids.eyes.length; e++) {
        ...
        ctx.fillRect(ex - rx, ey + ry - Math.max(1, ry * 0.24), rx * 2, Math.max(1, ry * 0.24));
      }
    }
  ...
  Puppet.prototype.debugPivots = function (B) {
    ...
    for (var i = 0; i < this.layers.length; i++) {
      ...
      ctx.beginPath();
      ctx.arc(x, y, W / 90, 0, Math.PI * 2);
      ctx.moveTo(x - W / 60, y); ctx.lineTo(x + W / 60, y);
      ctx.moveTo(x, y - W / 60); ctx.lineTo(x, y + W / 60);
      ctx.stroke();
      ctx.fillStyle = '#7cf6ff';
      ctx.fillText(L.role, x + W / 70, y - W / 120);
    }
  };

  Puppet.prototype.tick = function (now) {
    var dt = Math.min(0.034, Math.max(0, (now - this.last) / 1000));
    this.last = now;
      ...
      Promise.all(pending.map(function (p) {
        ...
        pngs.forEach(function (f) {
          ...
          layers.push({
            ...
            bend: /hair|cloth/i.test(role) ? 1 : 0
          });
        });
        ...
        probe.onerror = function () { reject(new Error('could not decode those PNGs')); };
        probe.src = layers[0].url;
      });
    });
  }

  /* Fetch a folder over http: pivots.json is optional, the file list is not. */
  function modelFromDir(dir, files) {
    ...
    return fetch(encodeURI(dir + 'pivots.json')).then(function (r) {
      ...
      var layers = files.map(function (f) {
        ...
        return { file: f, name: f, role: role, bend: /hair|cloth/i.test(role) ? 1 : 0 };
      });
      return new Promise(function (resolve, reject) {
        var probe = new Image();
        probe.onload = function () {
