...
$unityArgs=@('-batchmode','-projectPath','"F:\Resonance\client"','-giCustomCacheLocation','"F:\Resonance\client\Temp\GICache-D-batch"','-logFile',('"'+$evidenceRoot+'\'+$Name+'.log"'))
if($Mode -eq 'tests') {
    $unityArgs+=@('-runTests','-testPlatform','EditMode','-testFilter','Resonance.EditorTests.BattleTutorialPresentationTests','-testResults',('"'+$evidenceRoot+'\'+$Name+'.xml"'))
} elseif($Mode -eq 'render') {
    $env:RESONANCE_TUTORIAL_EVIDENCE=Join-Path $evidenceRoot ($Name+'-frames')
    $unityArgs+=@('-executeMethod','Resonance.EditorTools.TutorialPresentationVerification.RunAndExit')
} else {
